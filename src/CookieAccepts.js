...
function CookieAccepts() {
  ...
  const [randLeft, setRandLeft] = useState(Math.floor(Math.random() * document.body.clientWidth))
  ...
  return (
    <div className="CookieAccepts">
      <div className="w-screen h-screen bg-gradient-to-tl from-violet-400 via-indigo-600 to-fuchsia-500 flex justify-center items-center">
        <p className='text-6xl absolute font-bold text-opac z-0 duration-300' id='p' style={{'left': randLeft}}>TODO LIST</p>
        <p className='text-6xl absolute font-bold text-opac z-0 duration-300' id='p' style={{'left': randLeft}}>TODO LIST</p>
